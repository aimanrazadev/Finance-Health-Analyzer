...
def build_fallback_content(context:AIFinancialContext)->AIInsightsContent:
    ...
    recs=generate_recommendations(context);top=recs[0].title if recs else "Keep reviewing your spending"
    return AIInsightsContent(summary=f"Your financial health score for {context.period_label} is {context.health_score.overall_score}/100 ({context.health_score.status}). Your top priority is: {top}.",spending_insights=spending[:4],savings_insights=savings[:4],merchant_insights=merchants[:4],subscription_insights=subscriptions[:4],health_insights=health[:4],recommendations=recs)
