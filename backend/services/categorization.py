...
from models import Category, CategoryLearningRule, UserLearning
from services.ai_categorization_service import suggest_category_with_ai
from services.learning_service import create_or_update_learning_rule, increment_rule_usage
from services.merchant_extractor_service import extract_merchant_name, normalize_merchant_name
from services.ml_categorization_service import predict_category_with_ml
...
def _get_category_result(
    ...
    if confidence < SUGGESTION_CONFIDENCE_THRESHOLD:
    ...
    return {
        ...
        "suggested_category_name": category_name,
        "confidence": confidence,
        "method": method if confidence >= AUTO_SAVE_CONFIDENCE_THRESHOLD else "needs_review",
        "merchant": merchant,
        "requires_confirmation": confidence < AUTO_SAVE_CONFIDENCE_THRESHOLD,
    }
...
def _learned_rule_prediction(
    ...
    if exact_rule:
        ...
        return exact_rule, 0.95, "user_learned"
    ...
    if best_rule and best_score >= 0.78:
        ...
        return best_rule, 0.80, "user_learned"
...
def categorize_transaction(
    ...
    """Categorize with learned user rules, global rules, ML, then Needs Review."""
    extracted_merchant = merchant or extract_merchant_name(description)
    ...
    if user_id is not None:
        ...
        if ml_category_name and ml_confidence >= SUGGESTION_CONFIDENCE_THRESHOLD:
    ...
    ai_category_name, ai_confidence = suggest_category_with_ai(description, extracted_merchant, transaction_type)
    if ai_category_name and ai_confidence >= SUGGESTION_CONFIDENCE_THRESHOLD:
        return _get_category_result(db, ai_category_name, ai_confidence, "ai_fallback", extracted_merchant)
